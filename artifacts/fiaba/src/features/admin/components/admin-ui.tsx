...
export function AdminBadge({ children, tone = 'violet', className = '' }: { children: ReactNode; tone?: BadgeTone; className?: string }) {
...
export type AdminButtonVariant = keyof typeof buttonVariants;
...
/* ── SectionTitle ── */
export function AdminSectionTitle({ title, subtitle, action }: { title: string; subtitle?: string; action?: ReactNode }) {
  return (
...
export function AdminDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50" data-testid={testId} role="dialog" aria-modal="true">
      <div className="absolute inset-0 bg-[#201b3c]/75" onClick={onClose} data-testid="drawer-overlay" />
      ...
      <div className="relative flex min-h-full items-center justify-center p-4">
        <div className="relative w-full max-w-sm rounded-[24px] bg-[#fbfaff] p-6 shadow-xl">
          ...
          <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <AdminButton variant="ghost" onClick={onClose} testId="confirm-cancel">{cancelLabel}</AdminButton>
            <AdminButton variant={cfg.btn} onClick={() => { onConfirm(); onClose(); }} testId="confirm-ok">{confirmLabel}</AdminButton>
          </div>
        </div>
      </div>
    </div>
...
/* ── Toggle ── */
export function AdminToggle({ checked, onChange, testId }: { checked: boolean; onChange: (v: boolean) => void; testId?: string }) {
  return (
    <button
      type="button"
...
/* ── ProgressBar ── */
export function AdminProgressBar({ value, tone = 'violet' }: { value: number; tone?: 'violet' | 'mint' | 'amber' | 'rose' }) {
  const colors = { violet: 'bg-[#5b49e8]', mint: 'bg-[#278e69]', amber: 'bg-[#ac741e]', rose: 'bg-[#c45667]' };
  return (
...
/* ── SeverityBadge (admin-specific) ── */
export function SeverityBadge({ severity }: { severity: 'Critique' | 'Élevé' | 'Moyen' }) {
  const tone = severity === 'Critique' ? 'rose' : severity === 'Élevé' ? 'amber' : 'slate';
  return <AdminBadge tone={tone as BadgeTone}>{severity}</AdminBadge>;
}

/* ── TrustScore (compact pill) ── */
export function TrustScore({ score }: { score: number }) {
  const tone = score >= 75 ? 'mint' : score >= 50 ? 'amber' : 'rose';
  return <AdminBadge tone={tone as BadgeTone}>{score}/100</AdminBadge>;
}
