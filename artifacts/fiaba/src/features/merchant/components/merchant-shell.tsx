...
import { Cancel01Icon, Chart02Icon, CheckmarkCircle02Icon, Home01Icon, Logout01Icon, Menu02Icon, Notification01Icon, Search01Icon, Store01Icon } from '@hugeicons/core-free-icons';
...
const bottomNav = [
  { href: '/merchant', label: 'Accueil', glyph: Home01Icon },
...
export function MerchantShell({ children }: { children: ReactNode }) {
  ...
  const navLink = (item: (typeof allNav)[number], isMobile = false) => {
    ...
    return (
      <Link
        ...
        className={base}
        data-testid={`link-${isMobile ? 'mobile-' : ''}nav-${item.href.split('/').pop()}`}
      >
  ...
  return (
    <div className="merchant-noise merchant-shell min-h-[100dvh] text-[#282441]">
      {/* Desktop sidebar */}
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[242px] flex-col overflow-y-auto bg-[#242046] px-4 py-6 text-white lg:flex dark-scrollbar">
        ...
        <button onClick={() => handleSignOut()} className="mt-4 flex items-center gap-3 px-3 py-2 text-xs font-bold text-[#c1bdd8] hover:text-white" data-testid="button-logout">
      ...
      <header className="sticky top-0 z-30 flex h-[64px] items-center justify-between bg-[#f8f8fc] px-4 sm:px-5 lg:ml-[242px] lg:h-[70px] lg:px-9">
        <div className="flex items-center gap-3">
          <div className="lg:hidden">
            <MerchantLogo light={false} />
          </div>
          <div className="hidden lg:block">
            <p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#8b88a0]">Espace commerçant</p>
            <p className="mt-1 text-sm font-bold text-[#38324f]">{profile?.full_name ?? 'Maison Ndar'}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
          ...
          <button
            ...
            onClick={() => toast({ title: '3 nouvelles notifications', description: '2 commandes et 1 vendeur en attente de validation.' })}
      ...
      {mobile && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-[#201b3c]/75" onClick={() => setMobile(false)} data-testid="mobile-overlay" />
          <div className="absolute left-0 top-0 flex h-full w-[300px] flex-col bg-[#242046] px-4 py-6 text-white shadow-2xl">
            <div className="flex items-center justify-between">
              <MerchantLogo />
              <button onClick={() => setMobile(false)} className="rounded-lg p-1.5 text-[#c1bdd8] hover:bg-white/10" data-testid="button-close-mobile">
                <Icon glyph={Cancel01Icon} size={22} />
              </button>
            </div>
            <div className="mt-8 px-2 text-[10px] font-bold uppercase tracking-[.16em] text-[#9791c5]">Piloter le commerce</div>
            <nav className="mt-3 space-y-1">{primaryNav.map((item) => navLink(item, true))}</nav>
            <div className="mt-6 px-2 text-[10px] font-bold uppercase tracking-[.16em] text-[#9791c5]">Votre espace</div>
            <nav className="mt-3 space-y-1">{secondaryNav.map((item) => navLink(item, true))}</nav>
            <button onClick={() => handleSignOut()} className="mt-auto flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold text-[#c1bdd8] hover:bg-white/10" data-testid="button-mobile-logout">
              Quitter l'espace <Icon glyph={Logout01Icon} size={16} className="ml-auto" />
            </button>
          </div>
        </div>
      ...
      >
        ...
        {bottomNav.map((item) => {
          ...
          return (
            <Link
              ...
              data-testid={`link-bottom-${item.href.split('/').pop()}`}
            >
        ...
        <button
          ...
          data-testid="button-bottom-plus"
